...
impl CircleBounds {
    ...
    fn __repr__(&self) -> String{
        format!("<Circle: center ({:?}, {:?}) radius {:?}>", self.0.center_pt.0, self.0.center_pt.1, self.0.radius)
    }
}
...
impl ConvexPolygonBounds {
    ...
    fn new(pts: Vec<(f32, f32)>) -> PyResult<Self> {
        match rbound::ConvexPolygonBounds::new(&pts) {
            Some(e) => Ok(Self(e)),
            None => Err(PyRuntimeError::new_err("cannot build convex polygon from points"))
        }
    }
...
fn plan_rrt(
    ...
    let new_sphere: Vec<rbound::CircleBounds> = obstacles.iter().map(|(x, y, r)| rbound::CircleBounds{center_pt: (*x, *y), radius: *r}).collect();
    ...
    let mut rrt = rrt::RRT::new(start, goal, new_obstacles, expand_dis, path_resolution, goal_sample_rate, max_iter, new_explore_area);
